...
model_pth = r'/Users/gopalagarwal/PycharmProjects/SentinelAI/Vandalism.pt'
folder_pth = r'/Users/gopalagarwal/PycharmProjects/SentinelAI/Vandalism_detection'
...
def save_base64_and_send_webhook(new_img, webhook_url):
    ...
    response = requests.post(webhook_url, json={'picture': picture_url, 'model': 'vandalism', 'cameraIp': camera_ip})
...
while True:
    _, frame = cap.read()
    results = model.predict(frame, show=True, classes=[0], conf=.7)
    current = time.time()
